                ...
                switch (type) {
                    ...
                    case ALL_FARMS:
                        ...
                        provider.loadAllPotData(true, c1 -> {
                            if (c1) {
                                plugin.logger().info(" | Amount of " + plugin.farmManager().potManager().map().keySet().size() + " pot data loaded in " + FarmAPI.took(now.get()) + ".");
                                now.set(System.currentTimeMillis());
                                provider.loadAllCropData(true, c2 -> {
                                    if (c2) {
                                        plugin.logger().info(" | Amount of " + plugin.farmManager().cropManager().map().keySet().size() + " crop data loaded in " + FarmAPI.took(now.get()) + ".");
                                        now.set(System.currentTimeMillis());
                                        provider.loadAllGreenhouseData(true, c3 -> {
                                            if (c3) {
                                                plugin.logger().info(" | Amount of " + plugin.farmManager().greenhouseManager().map().keySet().size() + " greenhouse data loaded in " + FarmAPI.took(now.get()) + ".");
                                                now.set(System.currentTimeMillis());
                                                provider.loadAllScarecrowData(true, c4 -> {
                                                    if (c4) {
                                                        plugin.logger().info(" | Amount of " + plugin.farmManager().scarecrowManager().map().keySet().size() + " scarecrow data loaded in " + FarmAPI.took(now.get()) + ".");
                                                    } else {
                                                        plugin.logger().severe(" | Failed to load all scarecrow data!");
                                                    }

                                                    plugin.logger().info(" | All farm data loaded in " + FarmAPI.took(baseNow.get()));
